...
- Service — is an interface that defines a set of methods or operations. Concrete implementations of these services are
  ...
  - [RunConfiger](./interfaces.go#L9) / [PalRunConfiger](./interfaces.go#L17) — optional runner scheduling (`ShouldWaitForRunner() bool`: true = wait / main runner, false = fire-and-forget). Use `PalShouldWaitForRunner` when `ShouldWaitForRunner` is already taken.
  
  **Dispatch order** when more than one mechanism could apply: lifecycle hooks (`ToInit`, `ToShutdown`, `ToHealthCheck`) run first; then Pal-prefixed methods if implemented; then the standard `Init` / `Shutdown` / `HealthCheck` / `ShouldWaitForRunner` methods. For background work, `PalRun` is preferred over `Run` when both exist, and `PalShouldWaitForRunner` over `ShouldWaitForRunner` when both exist. In normal use, implement **one** style per phase (standard **or** Pal-prefixed), not both on purpose.
...
3. **Running**:
   ...
   - Runners that implement [RunConfiger](./interfaces.go#L9) or [PalRunConfiger](./interfaces.go#L17) supply scheduling via `ShouldWaitForRunner()` or `PalShouldWaitForRunner()` (Pal-prefixed wins if both are present); otherwise a default of wait/main applies for types that only implement `Run` / `PalRun`.
