...
func TestResolveWorkDirEmptyUsesCwd(t *testing.T) {
	...
	workDir, promoteTo, cleanup, err := resolveWorkDir("")
	require.NoError(t, err)
	defer cleanup()
	got, err := os.Getwd()
	require.NoError(t, err)
...
func TestResolveWorkDir(t *testing.T) {
	...
	t.Run("existing directory", func(t *testing.T) {
		...
		workDir, promoteTo, cleanup, err := resolveWorkDir(dir)
		require.NoError(t, err)
		defer cleanup()
	...
	t.Run("existing file errors", func(t *testing.T) {
		...
		_, _, cleanup, err := resolveWorkDir(file)
		cleanup()
		require.Error(t, err)
	...
	t.Run("missing path uses temp and promote target", func(t *testing.T) {
		...
		workDir, promoteTo, cleanup, err := resolveWorkDir(target)
		require.NoError(t, err)
		defer cleanup()
...
func TestPromoteWorkDir(t *testing.T) {
	...
	require.NoError(t, promoteWorkDir(tmp, target))
